...
public class TC003FullOtherPayment extends BaseClass {
	...
	@Test(testName = "testCreateBookingWithOtherPayment", priority = 1)
	public void testCreateBookingWithOtherPayment() throws IOException, InterruptedException {
		try {
			...
			handleException(e, "testCreateBookingWithOtherPayment");
		}
	}
	...
	private void verifyBookingSuccess() throws IOException, InterruptedException {
		if (driver.getPageSource().contains("There is some error in receiver's bank or outlet is not taking online payment.")) {
			...
			ExtentReportManager.logInfo("Booking created successfully by Other payment.");
		}
	}
